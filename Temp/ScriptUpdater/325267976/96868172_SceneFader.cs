...
public class SceneFader : MonoBehaviour
{
    ...
    private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        ...
        if (player != null)
        {
            ...
            finalPos.y -= 50f; // 👈 모든 씬에서 Y 위치에 -50f 오프셋 적용
            ...
            if (rb != null)
            {
                // 🛑 수정 완료: Rigidbody2D는 linearVelocity 대신 velocity를 사용합니다.
                rb.linearVelocity = Vector2.zero; 
                rb.angularVelocity = 0f;
            }
        
            Debug.Log($"플레이어 위치를 씬 로드 후 목표 위치 ({finalPos})로 이동 완료.");
        }
    }
