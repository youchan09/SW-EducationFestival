...
public class SceneFader : MonoBehaviour
{
    ...
    private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        ...
        if (scene.name == "GameClearScene")
        {
            ...
            if (player != null)
            {
                // ⭐️ 1. 부모 관계가 있다면 해제하여 월드 좌표를 정확히 따르도록 보장
                player.transform.SetParent(null); 
                
                // ⭐️ 2. 목표 위치로 즉시 이동
                player.transform.position = playerPos;
                
                // ⭐️ 3. Rigidbody가 있다면 잔여 속도를 제거하여 즉시 멈춤
                ...
                Debug.Log($"플레이어 위치를 {scene.name}의 목표 위치 ({playerPos})로 이동 완료.");
            }
        }
        
        // 여기에 다른 씬에 대한 위치 설정 로직을 추가할 수 있습니다.
    }
